...
def parse_response(response):
    ...
    for slide in slides:
        ...
        keyword_line = [line for line in lines if line.startswith('Keyword:') or line.startswith('Keywords:')]
        if keyword_line:
            keyword = keyword_line[0].split(': ', 1)[1]
        else:
            keyword = ""  # Assign an empty string if no keyword line is found
...
def create_ppt(slides_content, template_choice, presentation_title, presenter_name, insert_image):
    ...
    output_directory = r'change directory'

    # Save the presentation
    output_path = os.path.join(output_directory, 'generated_presentation.pptx')
